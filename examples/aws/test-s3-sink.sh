...
KEY="veridion/events/repo=acme_service-a/year=2026/month=05/day=14/ts=2026-05-14T12:00:00Z-pr=42.json"

PYTHONPATH=src .venv/bin/python3 -m veridion.action.decision_sinks \
  --decision-event-path "${EVENT_PATH}" \
  --sink "s3:bucket=${VERIDION_TEST_BUCKET},key=${KEY},region=${AWS_REGION}" \
